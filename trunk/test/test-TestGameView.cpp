...
TEST (TestGameView_event_handle)
{
    ...
    params1.id = 101;
    view->enqueue (new Event("LOGIC_STORE", &params1, sizeof(params1)));

    ActorStoreParams params2;
    params2.id = 102;
    view->enqueue (new Event("ACTOR_STORE", &params2, sizeof(params2)));

    ViewStoreParams params3;
    params3.id = 103;
    view->enqueue (new Event("VIEW_STORE", &params3, sizeof(params3)));

    ControllerStoreParams params4;
    params4.id = 104;
    view->enqueue (new Event("CONTROLLER_STORE", &params4, sizeof(params4)));

    // この呼び出しでビューが全イベントを処理し、
    // "LOGIC_STORE"と"ACTOR_STORE"をロジックへ転送する。
    view->update (0);
